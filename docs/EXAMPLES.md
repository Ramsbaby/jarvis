...
Jarvis가 자연어 요청을 받아 Kakao Calendar에 이벤트를 등록합니다.

**Discord 대화:**
```
User:  26년 5월 10일에 "휴대폰요금제프로모션종료" 카카오캘린더 등록해줘
Jarvis: ✅ 카카오 캘린더 등록 완료
        - 제목: 휴대폰요금제프로모션종료
        - 날짜: 2026-05-10 (종일)
        - event_id: 69baae64a9f57902c4055649
```

**내부 동작:**
```bash
bash scripts/kakao-calendar-add.sh "2026-05-10" "휴대폰요금제프로모션종료"
# → config/secrets/kakao.json에서 access_token 로드
# → 만료 시 kakao-token-refresh.sh 자동 실행
# → Kakao Calendar API 호출 (form-data, event 필드)
```
...
mcp__serena__write_memory({
  memory_name: "global/integrations/kakao-calendar",
  content: "# 카카오 캘린더\n- 토큰 위치: config/secrets/kakao.json\n..."
})
```

**검색:**
```
mcp__nexus__rag_search({ query: "카카오 캘린더 토큰 위치" })
→ [{ content: "# 카카오 캘린더\n...", score: 0.92 }]
```

**자연어 트리거:**
```
Discord: "저번에 카카오 연동 어떻게 했지?"
