...
```bash
# 1. 토큰 파일 존재 여부
ls $BOT_HOME/config/secrets/kakao.json

# 2. 수동 토큰 갱신 시도
bash $BOT_HOME/scripts/kakao-token-refresh.sh

# 3. 로그 확인
tail -20 $BOT_HOME/logs/kakao-token-refresh.log
```

refresh_token도 만료됐다면 (60일 주기) Kakao OAuth re-flow 필요:
```
https://kauth.kakao.com/oauth/authorize?client_id=<REST_API_KEY>&redirect_uri=https://localhost&response_type=code&scope=talk_calendar
```

---

### Q. 카카오 API가 `not allowed Content-Type` 오류를 반환합니다.

**A.** Kakao Calendar API는 `application/json` body를 허용하지 않습니다. `application/x-www-form-urlencoded` 형식으로 `event` 필드에 JSON 문자열을 전송해야 합니다.

```bash
curl -X POST "https://kapi.kakao.com/v2/api/calendar/create/event" \
  -H "Authorization: Bearer $ACCESS_TOKEN" \
  -H "Content-Type: application/x-www-form-urlencoded" \
  --data-urlencode "event={\"title\":\"테스트\", ...}"
```
...
> 더 궁금한 점은 [GitHub Issues](https://github.com/ramsbaby/jarvis/issues)에 등록해주세요.
