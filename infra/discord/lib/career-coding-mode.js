...
//   solve = 자바 직접 풀이 — 빠름 (Sonnet·6턴·도구 없음, 2026-06-07)
//   deep  = 정밀 풀이 — 확실함 (Opus·effort max·80턴·직접 컴파일/실행 검증, 시간 무관, 2026-06-11)
...
export function getCareerCodingMode() {
  try {
    ...
    if (m === 'solve' || m === 'coach' || m === 'deep' || m === 'off') return m;
...
export function setCareerCodingMode(mode, via = 'unknown') {
  if (mode !== 'coach' && mode !== 'solve' && mode !== 'deep' && mode !== 'off') {
...
export function parseCodingModeCommand(text) {
  ...
  if (/정밀|딥|deep|확실/i.test(t)) return 'deep';          // "정밀 풀이" 오인 방지 — deep을 solve보다 먼저
