...
# 재개: rm ~/.openclaw-data/runtime/state/stopped/duplicate-request-cache-monitor
if [[ -f "${HOME}/.openclaw-data/runtime/state/stopped/duplicate-request-cache-monitor" ]] && [[ "${OPENCLAW_JOB:-}" != "1" ]]; then
