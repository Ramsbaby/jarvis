...
test_db_initialization() {
    ...
    # 기존 DB 제거 (테스트용)
    rm -f ~/.openclaw-data/jarvis/runtime/state/command-state-cl-3e0048f79eb206f9.db

    # 초기화 함수 호출
    _init_sqlite_db

    # DB 파일 존재 여부 확인
    if [[ -f ~/.openclaw-data/jarvis/runtime/state/command-state-cl-3e0048f79eb206f9.db ]]; then
        echo "  ✓ DB 파일 생성됨"
    else
    ...
    echo "  2. 테이블 스키마 확인"
    local schema
    schema=$(sqlite3 ~/.openclaw-data/jarvis/runtime/state/command-state-cl-3e0048f79eb206f9.db ".tables" 2>/dev/null || echo "")
...
main() {
    ...
    rm -f ~/.openclaw-data/jarvis/runtime/state/command-state-cl-3e0048f79eb206f9.db
    mkdir -p ~/.openclaw-data/jarvis/runtime/state/cluster-guards
