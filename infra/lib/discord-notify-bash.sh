...
send_discord() {
    ...
    if [[ -n "$webhook" && "$webhook" != https://* ]]; then
        webhook=$(jq -r ".webhooks[\"$webhook\"] // empty" "${BOT_HOME:-$HOME/.openclaw-data/jarvis/runtime}/config/monitoring.json" 2>/dev/null)
    fi
    ...
    local _mon="${BOT_HOME:-$HOME/.openclaw-data/jarvis/runtime}/config/monitoring.json"
