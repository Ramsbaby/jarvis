...
EXTRACTOR="${BOT_HOME}/lib/requirement-extractor.mjs"
...
log_ledger() {
    ...
    # Build JSON manually to avoid multiline issues with jq formatted requirements
    local ledger_entry="{\"ts\":\"$(date -u +%FT%TZ)\",\"task\":\"$task_id\",\"status\":\"$status\""

    # Add requirements (convert to single-line JSON if needed)
    if [[ -n "$reqs" ]]; then
        local reqs_oneline=$(echo "$reqs" | tr '\n' ' ' | sed 's/  */ /g')
        ledger_entry+=",\"requirements\":$reqs_oneline"
    fi

    # Add missing array
    if [[ -n "$missing" ]]; then
        ledger_entry+=",\"missing\":$missing"
    fi

    # Add extra field
    if [[ -n "$extra" ]]; then
        ledger_entry+=",\"extra\":\"$extra\""
    fi
...
check_requirements_post() {
    ...
    if section_list=$(echo "$requirements" | jq -r '.sections[]? // empty'); then
        while IFS= read -r section; do
            [[ -z "$section" ]] && continue
            if ! grep -qi "$section" "$result_file" 2>/dev/null; then
        ...
        if [[ -n "$expected_scope" ]]; then
            case "$expected_scope" in
                all)
                    ...
                    if ! grep -qi "전체" "$result_file" 2>/dev/null && ! grep -qi "all" "$result_file" 2>/dev/null; then
                        missing+=("scope:full_application_unclear")
                    fi
                    ;;
                partial)
                    # Check for partial markers
                    if ! grep -qi "부분" "$result_file" 2>/dev/null && ! grep -qi "partial" "$result_file" 2>/dev/null && ! grep -qi "일부" "$result_file" 2>/dev/null; then
    ...
    if echo "$requirements" | jq -e '.completeness_check' >/dev/null 2>&1; then
        ...
        completeness_markers=$(grep -c "✓\|✅\|완료\|confirmed\|verified\|checked" "$result_file" 2>/dev/null || echo 0)
    ...
    if (( ${#missing[@]} > 0 )); then
        status="fail"
        # Build JSON array of missing items
        missing_json="["
        for i in "${!missing[@]}"; do
            [[ $i -gt 0 ]] && missing_json+=","
        ...
        if [[ "${REQUIREMENTS_STRICT:-0}" == "1" ]]; then
            ...
            log_ledger "$task_id" "validation_failed_strict" "$requirements" "$missing_json" ""
            return 1
        else
            log_ledger "$task_id" "validation_failed_warn" "$requirements" "$missing_json" ""
            return 0
        fi
    else
