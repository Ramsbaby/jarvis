...
#
# 사용:
#   source ~/projects/jarvis/infra/lib/discord-route.sh
...
_CHANNEL_MAP_GUARD="${HOME}/projects/jarvis/infra/guards/validate-channel-map.sh"
_EGRESS_AUDIT_LOG="${HOME}/.openclaw-data/runtime/logs/egress-audit.log"
...
_no_external() {
    ...
    local log="${BOT_HOME:-${HOME}/.openclaw-data/runtime}/logs/no-external.log"
...
_DISCORD_ROUTE_DEDUP_DIR="${HOME}/.openclaw-data/runtime/state/discord-route-dedup"
...
discord_route_raw() {
    ...
    [ -f "$monitoring" ] || monitoring="${HOME}/.openclaw-data/runtime/config/monitoring.json"
