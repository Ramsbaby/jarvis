...
# 실행: bash ~/.openclaw-data/jarvis/infra/lib/test-async-work-guard.sh
#
# 테스트 케이스:
#   1. 비동기 작업 상태 폴링 (성공)
#   2. 검증 상태 명시 보고
...
setup_test_env() {
    ...
    mkdir -p ~/.openclaw-data/jarvis/runtime/state/async-tasks
    mkdir -p ~/.openclaw-data/jarvis/runtime/logs/async
    mkdir -p ~/.openclaw-data/jarvis/runtime/state/verified-reports
    mkdir -p ~/.openclaw-data/jarvis/runtime/logs/verified-reports
}
...
test_report_template() {
    ...
    if [ -f "${HOME}/.openclaw-data/jarvis/runtime/state/verified-reports/${TEST_CLUSTER_ID}_${task_id}.report" ]; then
...
test_state_file_structure() {
    ...
    local report_file="${HOME}/.openclaw-data/jarvis/runtime/state/verified-reports/${TEST_CLUSTER_ID}_${task_id}.report"
