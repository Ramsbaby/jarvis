...
#
# 개선사항 (2026-05-13):
#   - Stage 1a oauth-refresh.sh 경로 실제 파일시스템에 symlink 생성 (~/.openclaw-data/runtime/scripts/)
...
#
# 통계:
#   ~/.openclaw-data/runtime/state/continue-sites-stats.json

set -eo pipefail
set +o pipefail  # stdout 닫힘으로 인한 SIGPIPE 방지 (명령 치환에서 cat 출력 시)

_CS_STATS_FILE="${BOT_HOME:-${HOME}/.openclaw-data/runtime}/state/continue-sites-stats.json"
_CS_STAGE_DELAY=5
...
run_with_recovery() {
    ...
    export PATH="${BOT_HOME:-${HOME}/.openclaw-data/runtime}/bin:/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:${HOME}/.local/bin:${PATH:-/usr/bin:/bin}"
