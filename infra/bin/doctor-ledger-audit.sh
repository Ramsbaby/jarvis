...
LEDGER="${HOME}/.openclaw-data/runtime/state/doctor-ledger.jsonl"
REPORT_LOG="${HOME}/.openclaw-data/runtime/logs/doctor-weekly-audit.log"
...
if command -v node >/dev/null 2>&1 && [ -f "${HOME}/projects/jarvis/infra/scripts/discord-visual.mjs" ]; then
  node "${HOME}/projects/jarvis/infra/scripts/discord-visual.mjs" \
    --type system-doctor \
    --data "$(jq -cn \
