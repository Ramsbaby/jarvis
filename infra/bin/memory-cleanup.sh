...
# === 1. Clean up old files in ~/.openclaw-data/runtime/results/ ===
...
# === 2. Clean up stale sessions in ~/.openclaw-data/runtime/state/sessions.json ===
...
# === 3. Clean up old event files in ~/.openclaw-data/runtime/state/events/ ===
...
# === 4. Clean up old sentinel files in ~/.openclaw-data/runtime/state/active-tasks/ ===
...
| 항목 | 정리됨 |
|------|--------|
| **결과 파일** (~/.openclaw-data/runtime/results) | $RESULTS_CLEANED |
| **세션 항목** (sessions.json) | $SESSIONS_CLEANED |
| **이벤트 파일** (~/.openclaw-data/runtime/state/events) | $EVENTS_CLEANED |
| **Sentinel 파일** (~/.openclaw-data/runtime/state/active-tasks) | $SENTINELS_CLEANED |
