...
#
# 호출:
#   bash ~/projects/jarvis/infra/bin/worktree-sync.sh          # 실 merge
#   bash ~/projects/jarvis/infra/bin/worktree-sync.sh --dry-run # 대상만 출력
#
# 스케줄 권고:
#   매일 00:00 KST (LaunchAgent ai.jarvis.worktree-sync 또는 crontab)
#
# 로그: ~/.openclaw-data/runtime/logs/worktree-sync.log (append-only, daily 로테이션 권장)
