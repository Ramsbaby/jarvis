...
# 결과 JSON: ~/.openclaw-data/runtime/state/claude-md-audit.json
...
# 단순화: ~/.jarvis/scripts/*.sh / ~/projects/jarvis/infra/bin/*.sh 호출 grep만  # ALLOW-DOTJARVIS
BROKEN_CMDS=0
DEAD_REFS=()
for f in "${CANDIDATES[@]}"; do
  [[ -f "$f" ]] || continue
...
if (( ${#STALE_FILES[@]} > 3 )); then STATUS="WARN"; WARNINGS+=("60 days+ stale files: ${#STALE_FILES[@]}"); fi
if (( DUP_LINES > 10 )); then STATUS="WARN"; WARNINGS+=("duplicate lines: $DUP_LINES"); fi
if (( BROKEN_CMDS > 0 )); then STATUS="WARN"; WARNINGS+=("dead script refs: $BROKEN_CMDS"); fi
...
log "result: total=${TOTAL_KB}KB / stale=${#STALE_FILES[@]} / dup=$DUP_LINES / broken=$BROKEN_CMDS / status=$STATUS"
[[ "$STATUS" != "OK" ]] && for w in "${WARNINGS[@]}"; do log "  WARNING: $w"; done
