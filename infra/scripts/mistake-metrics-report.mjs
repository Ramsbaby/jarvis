...
//
// 실행: node ~/projects/jarvis/infra/scripts/mistake-metrics-report.mjs
// 기록: ~/.openclaw-data/runtime/ledger/correction-metrics.jsonl (append — 회차 간 비교용)
...
const HOME = homedir();
const MISTAKES = join(HOME, '.openclaw-data/runtime/wiki/meta/learned-mistakes.md');
const AUTOLEARN = join(HOME, '.claude/rules/jarvis-autolearn.md');
const PROMOTER_LEDGER = join(HOME, '.openclaw-data/runtime/ledger/promoter-ledger.jsonl');
const INJECT_LOG = join(HOME, '.openclaw-data/runtime/logs/context-state-inject.jsonl');
const OUT_LEDGER = join(HOME, '.openclaw-data/runtime/ledger/correction-metrics.jsonl');
