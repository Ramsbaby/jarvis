...
for f in "${PATHS[@]}"; do
  ...
  if [[ "$size" -lt "$WARN_BYTES" ]]; then continue; fi
  mb=$((size / 1024 / 1024))
  level=warn
  if [[ "$size" -ge "$CRIT_BYTES" ]]; then level=crit; fi
  printf '{"ts":"%s","path":"%s","bytes":%d,"mb":%d,"level":"%s"}\n' \
    "$TS_ISO" "$f" "$size" "$mb" "$level" >> "$LEDGER"
