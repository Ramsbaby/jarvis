...
#
# Purpose:
#   Tier 0 원장(`~/.openclaw-data/jarvis/runtime/state/token-ledger.jsonl`) 위에서 주간 패턴 감사.
...
#
# Output: Markdown 리포트 ~/.openclaw-data/jarvis/runtime/results/token-ledger-audit/<YYYY-MM-DD>.md
...
if [[ -n "$cb_high_fails" ]]; then
    recs="${recs}
### 서킷브레이커 해소
3회+ 연속실패 태스크의 root cause 파악 (\`~/.openclaw-data/jarvis/runtime/logs/claude-stderr-<task>*.log\` 참조).
"
fi
...
- [x] **Tier 0**: 토큰 원장 (`~/.openclaw-data/jarvis/runtime/state/token-ledger.jsonl`)
- [ ] **Tier 1**: 글로벌 일일 캡
