...
# 재개: rm ~/.openclaw-data/runtime/state/stopped/self-evolution-weekly
if [[ -f "${HOME}/.openclaw-data/runtime/state/stopped/self-evolution-weekly" ]] && [[ "${OPENCLAW_JOB:-}" != "1" ]]; then
...
#
# 사용(cron):
#   30 9 * * 1 BOT_HOME=$HOME/.openclaw-data/runtime /bin/bash $HOME/projects/jarvis/infra/scripts/self-evolution-weekly.sh \
#     >> $HOME/.openclaw-data/runtime/logs/self-evolution-weekly.log 2>&1
#
