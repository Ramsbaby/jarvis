...
#
# Usage (crontab):
#   5 8 * * * /bin/bash ~/.openclaw-data/runtime/scripts/smart-standup.sh >> ~/.openclaw-data/runtime/logs/company-agent.log 2>&1
...
if [[ "$ONLINE" == "true" ]]; then
    ...
    [[ -f "$_FRESHNESS_GUARD" ]] || _FRESHNESS_GUARD="${HOME}/projects/jarvis/infra/lib/data-freshness-guard.sh"
