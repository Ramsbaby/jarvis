...
// 재개: rm ~/.openclaw-data/runtime/state/stopped/rule-effectiveness-audit
import { existsSync as __sc } from 'node:fs';
import { homedir as __sh } from 'node:os';
if (__sc(__sh() + '/.openclaw-data/runtime/state/stopped/rule-effectiveness-audit') && process.env.OPENCLAW_JOB !== '1') {
...
const HOME = homedir();
const PROMOTER = `${HOME}/.openclaw-data/runtime/ledger/promoter-ledger.jsonl`;
const MISTAKES = `${HOME}/.openclaw-data/runtime/state/mistake-ledger.jsonl`;
const OUT_LEDGER = `${HOME}/.openclaw-data/runtime/state/rule-effectiveness.jsonl`;
...
const GATE_PROPOSALS = `${HOME}/.openclaw-data/runtime/state/gate-proposals.jsonl`;
const EMBED_CACHE = `${HOME}/.openclaw-data/runtime/state/rule-eff-embed-cache.json`;
const MONITORING = `${HOME}/.openclaw-data/runtime/config/monitoring.json`;
