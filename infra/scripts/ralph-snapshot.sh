...
#
# 출력: ~/.openclaw-data/runtime/state/snapshots/<label>-<YYYYMMDD-HHMM>/
...
SNAP_DIR="${HOME}/.openclaw-data/runtime/state/snapshots/${LABEL}-${TS}"
STATE_DIR="${HOME}/.openclaw-data/runtime/state"
CURATED_DIR="${HOME}/.openclaw-data/runtime/wiki/05-career/interview-curated"
...
GIT_HEAD="$(cd "${HOME}/projects/jarvis" && git rev-parse HEAD 2>/dev/null || echo unknown)"
GIT_BRANCH="$(cd "${HOME}/projects/jarvis" && git rev-parse --abbrev-ref HEAD 2>/dev/null || echo unknown)"
...
ln -sfn "$SNAP_DIR" "${HOME}/.openclaw-data/runtime/state/snapshots/latest-${LABEL}"
