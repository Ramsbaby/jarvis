...
#   RAG 인덱싱에서 누락됐다. 당시엔 원인을 "~/.jarvis 가 ~/.openclaw-data/jarvis/runtime 심링크라서"로
#   기록했으나, 2026-07-25 실측 결과 ~/.jarvis 는 심링크가 아니라 독립 실제 디렉터리였다.
#   실제 그림자는 두 경로로 생긴다:
#
#   A형) ~/.jarvis/runtime/...  ← 스크립트에 이 경로가 하드코딩된 경우.  # ALLOW-DOTJARVIS
#        정규 데이터는 ~/.openclaw-data/jarvis/runtime/ 에 있으므로 여기 쌓이면 RAG·감사에서 누락된다.
#
#   B형) ~/.openclaw-data/jarvis/runtime/runtime/...  ← 스크립트는 "$JARVIS_HOME/runtime/..." 을 쓰는데
#        JARVIS_HOME(저장소 루트 = ~/.openclaw-data/jarvis)이 BOT_HOME(한 단계 아래 = ~/.openclaw-data/jarvis/runtime)으로
