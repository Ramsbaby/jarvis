...
// 재개: rm ~/.openclaw-data/runtime/state/stopped/interview-star-diversity-audit
import { existsSync as __stopChk } from 'node:fs';
import { homedir as __stopHome } from 'node:os';
if (__stopChk(__stopHome() + '/.openclaw-data/runtime/state/stopped/interview-star-diversity-audit')) {
...
const FEED = process.env.DIVERSITY_FEED || join(HOME, '.openclaw-data/runtime/state/channel-feed/jarvis-interview.jsonl');
const SELF_LEDGER = process.env.DIVERSITY_LEDGER || join(HOME, '.openclaw-data/runtime/ledger/interview-diversity-audit.jsonl');
...
const INSIGHTS = process.env.DIVERSITY_INSIGHTS || join(HOME, '.openclaw-data/runtime/state/ralph-insights.jsonl');
...
if (!dryrun && violations.length) {
  try {
    execFileSync('node', [join(HOME, 'projects/jarvis/infra/lib/task-store.mjs'), 'enqueue',
  ...
  try {
    ...
    execFileSync('bash', [join(HOME, 'projects/jarvis/infra/scripts/alert-send.sh'), 'warning', 'jarvis-interview', '면접봇 분포 감사', msg.slice(0, 1400)], { timeout: 15000 });
