 ...
 *
 * Reads ~/.openclaw-data/runtime/config/tasks.json and emits:
...
const RUNTIME_HOME = process.env.BOT_HOME
  // [회차8 2026-09-12] JARVIS_HOME 하위에서 runtime 을 파생하면 안 된다 —
  //   저장소에는 runtime 이 없고 그 자리는 장벽 파일이다. 런타임 정본을 직접 쓴다.
  || path.join(HOME, '.openclaw-data', 'runtime');
...
function main() {
  ...
  md.push('>');
  md.push('> Source: `~/.openclaw-data/runtime/config/tasks.json` · Team mapping: mirror of `~/jarvis-board/lib/map/team-registry.ts`');
  md.push('');
  ...
  md.push('');
  md.push('```bash');
  md.push('node ~/projects/jarvis/infra/scripts/gen-tasks-index.mjs');
  md.push('```');
  md.push('');
