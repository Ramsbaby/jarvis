...
GUARD_STATE_DIR="${HOME}/.openclaw-data/runtime/state/cluster-guards/${CLUSTER_ID}"
GUARD_LOG="${HOME}/.openclaw-data/runtime/logs/cluster-completion-guard-${CLUSTER_ID}.log"
GUARD_REPORT="${HOME}/.openclaw-data/runtime/reports/cluster-completion-guard-${CLUSTER_ID}.json"
...
record_guard_execution() {
    ...
    if [[ -f "${HOME}/projects/jarvis/infra/lib/mistake-cluster-guard.mjs" ]]; then
        if [[ "$passed" == "false" ]]; then
            node "${HOME}/projects/jarvis/infra/lib/mistake-cluster-guard.mjs" \
