...
const HOME     = homedir();
const SCN_PATH = join(HOME, '.openclaw-data/runtime/state/scenarios/samsung-cnt.json');
const INSIGHTS = join(HOME, '.openclaw-data/runtime/state/ralph-insights.jsonl');
const FORBID   = join(HOME, '.openclaw-data/runtime/state/ralph-forbid-list.json');
const ROUNDS   = join(HOME, '.openclaw-data/runtime/state/ralph-rounds.jsonl');
const DOC_PATH = join(HOME, 'projects/jarvis/infra/docs/INTERVIEW-BOT.md');
const PROF_PATH = join(HOME, '.openclaw-data/runtime/context/interview-bot-profile.md');
// P5 fix: JSON.parse(URL문자열) → SyntaxError → null 버그 수정. 단순 문자열 추출로 변경.
const WEBHOOK  = process.env.DISCORD_INTERVIEW_WEBHOOK ||
                 (() => { try {
                   const line = readFileSync(join(HOME, '.openclaw-data/runtime/.env'), 'utf-8')
...
function checkC6() {
  ...
  if (existsSync(DOC_PATH)) {
    ...
    const runnerPath = join(homedir(), 'projects/jarvis/infra/scripts/interview-ralph-runner.mjs');
    const fastPath   = join(homedir(), 'projects/jarvis/infra/discord/lib/interview-fast-path.js');
...
function checkC7() {
  ...
  const RUNNER_PATH   = join(HOME, 'projects/jarvis/infra/scripts/interview-ralph-runner.mjs');
  const VERIFIER_PATH = join(HOME, 'projects/jarvis/infra/scripts/interview-verifier-server.mjs');
...
function checkC8() {
  ...
  const VERIFIER_PATH = join(HOME, 'projects/jarvis/infra/scripts/interview-verifier-server.mjs');
  const SCORER_PATH   = join(HOME, 'projects/jarvis/infra/scripts/interview-relevance-scorer.mjs');
