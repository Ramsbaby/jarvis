...
#
# 출력: ~/.openclaw-data/runtime/state/system-memory-trend.jsonl (시계열 누적)
#       ~/.openclaw-data/runtime/logs/system-memory-trend.log
...
LOG="${HOME}/.openclaw-data/runtime/logs/system-memory-trend.log"
STATE="${HOME}/.openclaw-data/runtime/state/system-memory-trend.jsonl"
...
if [[ -n "$CRIT_REASON" ]]; then
  ...
  if source "${HOME}/projects/jarvis/infra/lib/discord-route.sh" 2>/dev/null; then
