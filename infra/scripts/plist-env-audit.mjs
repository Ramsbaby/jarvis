...
const RULES = [
  {
    ...
    expectedSources: [
      { file: join(HOME, 'projects/jarvis/infra/discord/lib/interview-fast-path.js'), pattern: /CHANNEL_NAME\s*=\s*['"]([^'"]+)['"]/ },
  ...
  },
  {
    ...
    plistFile: join(HOME, 'projects/jarvis/infra/discord/lib/slash-proxy.js'),
    ...
    expectedSources: [
      ...
      { file: join(HOME, 'projects/jarvis/infra/discord/personas.json'), pattern: /"(149[0-9]{16})"\s*:/ },
    ],
  },
];
...
if (NOTIFY) {
  const notifyScript = join(HOME, 'projects/jarvis/infra/scripts/discord-visual.mjs');
