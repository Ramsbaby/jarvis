...
LEDGER="${HOME}/.openclaw-data/runtime/ledger/oauth-main-observer.jsonl"
...
if [[ "$HTTP" == "401" ]]; then
  ...
  if [[ ! -f "$_cd" ]] || (( $(date +%s) - $(stat -f %m "$_cd" 2>/dev/null || echo 0) > 3600 )); then
    touch "$_cd"
    bash "${HOME}/.openclaw-data/runtime/scripts/alert.sh" critical "🧟 메인 토큰 401 + 좀비세션" "$_msg" 2>/dev/null || true
  fi
