...
const KOREAN_FUNCTIONAL = new Set([
  ...
  // P4 추가: Q&A가이드에서 발견된 누락 패턴
  '정산','계약','입찰','현장','호실','사이','있으나','계정','로깅','함께',
  // 형용사/부사/수식어
  '기술적','구조적','없이','미리보기','있으나',
  // 기능어 (기술명사 아님)
  'Boot','모니터링',' 회피','드론','예외',
...
function extractDotList(text) {
  ...
  for (const g of groups) {
    g.split(/[·,]\s*/).forEach(item => {
      const t = item.trim();
      // 기능어 제외, 2자 이상, 순수 숫자 제외, 한국어 동사/형용사 과거형 제외 (-한, -었, -을 접미사)
      if (t.length >= 2 &&
          !KOREAN_FUNCTIONAL.has(t) &&
          !/^\d+$/.test(t) &&
          !/[한었을]$/.test(t)) {  // 동사 과거형/완료형 어미 제외
        items.push(t);
      }
    });
  }
