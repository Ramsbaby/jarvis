...
#     ~/.openclaw-data/jarvis/runtime/scripts/webhook-listener.sh
#
# 직접 HTTP 서버만 기동 (smee 없이):
#   WEBHOOK_PORT=9000 GITHUB_WEBHOOK_SECRET=mysecret \
#     ~/.openclaw-data/jarvis/runtime/scripts/webhook-listener.sh --no-smee
...
if [[ "$NO_SMEE" == false ]]; then
    if [[ -z "${JARVIS_SMEE_URL:-}" ]]; then
        ...
        log "  4. ~/.openclaw-data/jarvis/runtime/config/webhook.env 파일에 아래 내용 추가:"
