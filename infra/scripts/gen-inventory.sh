...
# 재개: rm ~/.openclaw-data/runtime/state/stopped/gen-inventory
if [[ -f "${HOME}/.openclaw-data/runtime/state/stopped/gen-inventory" ]] && [[ "${OPENCLAW_JOB:-}" != "1" ]]; then
...
#
# crontab 등록: 30 4 * * * ~/.openclaw-data/runtime/scripts/gen-inventory.sh >> ~/.openclaw-data/runtime/logs/gen-inventory.log 2>&1
# ============================================================

TASKS_JSON="$HOME/.openclaw-data/runtime/config/tasks.json"
MONITORING_JSON="$HOME/.openclaw-data/runtime/config/monitoring.json"
