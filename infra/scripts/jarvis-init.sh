...
#   bash ~/.openclaw-data/runtime/scripts/jarvis-init.sh

set -euo pipefail
export PATH="/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:${PATH}"

JARVIS_HOME="${JARVIS_HOME:-${HOME}/.openclaw-data/runtime}"
...
# ── 4. 실행 권한 일괄 설정 ───────────────────────────────────────────────────
step "실행 권한 설정"
# -L 필수: scripts/ bin/ 은 ~/projects/jarvis/infra/ 로 가는 심볼릭 링크라
