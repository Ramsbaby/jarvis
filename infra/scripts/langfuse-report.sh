...
# 재개: rm ~/.openclaw-data/runtime/state/stopped/langfuse-report
if [[ -f "${HOME}/.openclaw-data/runtime/state/stopped/langfuse-report" ]]; then
...
cd ~ && node ~/.openclaw-data/runtime/scripts/discord-visual.mjs \
  --type stats \
  --data "{
