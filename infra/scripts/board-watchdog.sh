...
# 원장: $HOME/.openclaw-data/runtime/state/board-watchdog.jsonl (10MB rotate).
#
# 환경변수:
#   BOARD_EXT_URL   외부 URL (필수). ~/.openclaw-data/runtime/.env 또는 shell profile에서 export.
#   BOARD_TUNNEL_NAME cloudflared 터널 이름 (기본: jarvis-board)
#
# LaunchAgent ai.jarvis.board-watchdog (StartInterval 300)로 기동.
set -euo pipefail

# 운영용 env 자동 로드 — SSoT 경로(~/.openclaw-data/runtime/.env) 직접 참조.
# ~/.jarvis는 ~/.openclaw-data/runtime 심링크라 둘 다 동작하지만 SSoT 일관성을 위해 실제 경로 사용.
# source 시 발생하는 stderr(파싱 에러 등)은 별도 로그로 격리해 watchdog 로그를 더럽히지 않음.
ENV_FILE="${HOME}/.openclaw-data/runtime/.env"
ENV_ERR_LOG="${HOME}/.openclaw-data/runtime/logs/env-load-errors.log"
if [ -f "$ENV_FILE" ]; then
...
LEDGER="${HOME}/.openclaw-data/runtime/state/board-watchdog.jsonl"
THROTTLE_DIR="${HOME}/.openclaw-data/runtime/state/board-watchdog-throttle"
...
if [ -z "$EXT_URL" ]; then
  echo "[ERROR] BOARD_EXT_URL 환경변수 미설정. ~/.openclaw-data/runtime/.env 에 추가하세요." >&2
  exit 1
fi
