...
# 출력:   ~/.openclaw-data/jarvis/runtime/docs/SYSTEM-OVERVIEW.md
#
...
if [[ -z "${BOT_HOME:-}" ]]; then
  if [[ -d "${HOME}/.openclaw-data/jarvis/runtime" ]]; then
...
import json, os
B = os.environ.get('BOT_HOME', os.path.expanduser('~/.openclaw-data/jarvis/runtime'))
