...
DISCORD_ROUTE_LIB="${HOME}/projects/jarvis/infra/lib/discord-route.sh"
...
run_tests() {
    ...
    # 워크플로우 스크립트 존재 확인
    if [[ -x "${HOME}/projects/jarvis/infra/scripts/task-completion-workflow.sh" ]]; then
        _test_pass
    else
    ...
    set +e  # exit code 보존
    "${HOME}/projects/jarvis/infra/scripts/task-completion-workflow.sh" \
