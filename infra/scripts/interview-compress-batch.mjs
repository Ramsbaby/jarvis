...
const SCENARIOS_DIR = join(homedir(), '.openclaw-data/runtime/state/scenarios');
const USER_PROFILE_PATH = join(homedir(), '.openclaw-data/runtime/context/user-profile.md');
...
async function main() {
  ...
  try {
    ...
    console.error(`❌ verifier-server 미가동 — 먼저 기동하십시오: bash ~/.openclaw-data/runtime/scripts/interview-ralph-start.sh`);
    process.exit(1);
  }
