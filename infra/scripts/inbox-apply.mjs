#!/usr/bin/env node
/**
 * Jarvis Inbox Apply — Chrome 확장 자동 폼 응답
 *
 * Usage:
 *   node inbox-apply.mjs <URL>                    # URL 직접 응답
 *   node inbox-apply.mjs <기관명 또는 키워드>     # matched.json에서 검색 후 응답
 *   node inbox-apply.mjs                          # matched.json 전체 목록 출력
 *
 * Examples:
 *   node inbox-apply.mjs https://example.com/listing/123
 *   node inbox-apply.mjs "<키워드>"
 */
...
function searchItem(query) {
  ...
  const results = matched.results.filter(item => {
    const haystack = `${item.org || item.company || ''} ${item.title}`.toLowerCase();
...
const sendDiscord = (content) => discordSend(content, 'jarvis-system', { username: 'Jarvis Inbox Apply' });

// ── Claude in Chrome 프롬프트 생성 ───────────────────────────────────────
function buildChromePrompt(applicant, jobInfo) {
  const b = applicant.basic;
  const c = applicant.experience || applicant.career;
  const e = applicant.education;
  const m = applicant.military;
  const d = applicant.diversity;
  const a = applicant.applyDefaults;

  return `현재 Chrome에 열려있는 폼 페이지에서 "지원하기" 또는 "Apply" 버튼을 찾아 클릭한 후, 아래 정보로 폼을 채워줘.

## 항목 정보
- 기관: ${jobInfo.org || jobInfo.company || '(알 수 없음)'}
...
2. 검색형 입력 필드 (학교명, 전공, 기관명 등): 텍스트 입력 → 나타난 검색 결과 항목을 **반드시 클릭**해서 확정.
3. 라디오 버튼 (신입/경력, 병역 대상/비대상): 해당 항목을 클릭.
4. 날짜 필드 (YYYY.MM): 입학/졸업일을 정확한 포맷으로 입력.
5. 경력 선택: "경력" 라디오 버튼 클릭.
6. 프로필 PDF 파일 업로드: ${applicant.files.resumePdf} 경로의 파일 업로드.
7. 포트폴리오: "${a.portfolioType}" 방식 선택 → ${a.portfolioUrl} 입력.
8. 경로 질문: "${a.applyChannel}" 선택.
9. 개인정보 동의: "필수" 표시된 체크박스만 모두 체크. 선택은 건너뛰어도 됨.
10. 요약/동기 textarea가 있으면 위 "요약" 내용 기반으로 3~5줄 작성.

## ⚠️ 절대 금지
- 제출/완료/Submit 버튼 클릭 금지
- 폼 채움만 완료하고 멈춰야 함
...
async function applyViaChrome(applicant, jobInfo) {
  console.log(`\n🚀 응답 시작: ${jobInfo.org || jobInfo.company || ''} - ${jobInfo.title || ''}`);
  console.log(`   URL: ${jobInfo.url}\n`);

  // 중복 체크
  if (isAlreadyApplied(jobInfo.url)) {
    console.log('⚠️  이미 응답한 항목입니다. 중단.');
    return;
  }

  // 1. Chrome에서 항목 페이지 열기
  console.log('🖥️  Chrome에서 항목 페이지 열기...');
  ...
  await sendDiscord(
    `📝 **폼 자동 채움 완료 (Claude in Chrome)**\n` +
    `기관: ${jobInfo.org || jobInfo.company || '?'}\n` +
    `포지션: ${jobInfo.title || '?'}\n` +
    `URL: ${jobInfo.url}\n\n` +
...
// ── 목록 출력 ───────────────────────────────────────────────────────────
function printList(results, title = '📋 매칭된 항목') {
  console.log(`\n${title} (${results.length}건)\n`);
  results.slice(0, 30).forEach((item, i) => {
    const icon = item.score >= 80 ? '🟢' : item.score >= 60 ? '🟡' : '⚪';
    const applied = isAlreadyApplied(item.url) ? ' [응답완료]' : '';
    console.log(`${i + 1}. ${icon} ${item.score}점 [${item.org || item.company}] ${item.title}${applied}`);
...
async function main() {
  ...
  console.log(`🚀 Jarvis Inbox Apply (지원자: ${applicant.basic.name})`);
  ...
  if (!arg) {
    ...
    if (!matched?.results) {
      console.error('❌ matched.json이 없습니다. 먼저 수집/스코어링을 실행하세요.');
      console.error('   cd ~/projects/jarvis/infra && node scripts/inbox-crawl.mjs && node scripts/inbox-match.mjs --detail');
      process.exit(1);
    }
    printList(matched.results.filter(j => j.score >= 60), '📋 매칭 결과 (60점 이상)');
    console.log('\n💡 사용법:');
    console.log('   node inbox-apply.mjs <URL>                # URL 직접 응답');
    console.log('   node inbox-apply.mjs <키워드>             # 키워드 검색 후 응답');
    console.log('   node inbox-apply.mjs "<여러 키워드>"       # AND 검색');
    return;
  }
  ...
  if (arg.startsWith('http://') || arg.startsWith('https://')) {
    const matched = loadMatched();
    const item = matched?.results?.find(j => j.url === arg) || { url: arg };
    await applyViaChrome(applicant, item);
    return;
  }

  // 기관명/키워드 검색
  const results = searchItem(arg);
  ...
  // 여러 건 → 목록 출력 후 첫 번째 자동 응답 X, 사용자 확인 필요
