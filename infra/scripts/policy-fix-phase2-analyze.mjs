...
const HOME = homedir();
const TASKS = JSON.parse(readFileSync(join(HOME, '.openclaw-data/runtime/config/tasks.json'), 'utf-8'));
const MGR = JSON.parse(readFileSync(join(HOME, '.openclaw-data/runtime/state/cron-manager.json'), 'utf-8'));
const MANIFEST_PATH = join(HOME, '.openclaw-data/runtime/state/policy-fix-manifest.csv');
