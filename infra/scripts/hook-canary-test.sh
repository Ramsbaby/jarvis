...
# 재개: rm ~/.openclaw-data/runtime/state/stopped/hook-canary-test
if [[ -f "${HOME}/.openclaw-data/runtime/state/stopped/hook-canary-test" ]] && [[ "${OPENCLAW_JOB:-}" != "1" ]]; then
...
LEDGER="${HOME}/.openclaw-data/runtime/ledger/hook-canary.jsonl"
ALERT="${HOME}/projects/jarvis/infra/scripts/alert-send.sh"
