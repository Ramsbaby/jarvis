...
# ③ 심링크 무결성 (타겟 비교)
echo "═══ ③ 심링크 무결성 ═══"
find ~/.openclaw-data/runtime -type l ! -path '*node_modules*' ! -path '*.bak*' 2>/dev/null | while read l; do
  printf "%s -> %s\n" "${l#$HOME/}" "$(readlink "$l")"
done | sort > /tmp/symlinks.after
broken=$(find ~/.openclaw-data/runtime -type l ! -path '*.bak*' -exec test ! -e {} \; -print 2>/dev/null | head -5)
if [[ -n "$broken" ]]; then
  echo "❌ 깨진 심링크:"
  echo "$broken" | sed 's/^/    /'
  fail=1
...
if tail -100 ~/.openclaw-data/runtime/logs/discord-bot.log 2>/dev/null | grep -q "Logged in as"; then
  echo "✅ Discord 로그인 확인"
  tail -5 ~/.openclaw-data/runtime/logs/discord-bot.log | sed 's/^/    /'
else
  echo "❌ 최근 로그인 기록 없음"
  fail=1
fi
...
/bin/bash ~/projects/jarvis/infra/scripts/symlink-topology-audit.sh || fail=1
echo ""
...
if [[ "$AUTO_MODE" == "1" ]]; then
  ...
  if [[ $fail -eq 0 ]]; then
    ...
    TITLE="⚠️ 재부팅 복구 누락"
    DATA="{\"title\":\"${TITLE}\",\"data\":{\"누락_LaunchAgent\":\"${MISSING_SUMMARY:-없음}\",\"로그\":\"~/.openclaw-data/runtime/logs/reboot-verify.log\",\"조치\":\"로그 확인 후 수동 bootstrap\"},\"timestamp\":\"${TS}\"}"
  fi
