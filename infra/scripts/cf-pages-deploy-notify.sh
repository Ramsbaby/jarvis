...
# Cloudflare Pages 배포 상태를 폴링해 변화 시 Discord(#jarvis-career)로 알림.
...
[ -z "${CF_KEY:-}" ] && { echo "no CF key — skip"; exit 0; }
...
def f(o):
    if isinstance(o,dict):
        for k,v in o.items():
            if k=='jarvis-career' and isinstance(v,str) and 'webhook' in v: return v
            r=f(v)
            if r: return r
...
print(f(d) or '')
" 2>/dev/null || true)
[ -z "${WEBHOOK:-}" ] && { echo "no webhook — skip"; exit 0; }
...
[ -z "$ACCT_RESP" ] && { echo "account API 호출 실패 — skip"; exit 0; }
ACCT_CODE=$(echo "$ACCT_RESP" | tail -1)
ACCT_JSON=$(echo "$ACCT_RESP" | sed '$d')
[ "$ACCT_CODE" != "200" ] && { echo "account 조회 실패 (HTTP $ACCT_CODE) — skip"; exit 0; }
validate_json "$ACCT_JSON" || { echo "account 응답 JSON 파싱 오류 — skip"; exit 0; }
ACCT=$(echo "$ACCT_JSON" | jq -r '.result[0].id' 2>/dev/null)
[ -z "$ACCT" ] || [ "$ACCT" = "null" ] && { echo "account ID 조회 실패 — skip"; exit 0; }
...
[ -z "$ID" ] || [ "$ID" = "null" ] && { echo "배포 ID 추출 실패 — skip"; exit 0; }
[ -z "$STAGE" ] || [ "$STAGE" = "null" ] && { echo "배포 단계 추출 실패 — skip"; exit 0; }
[ -z "$STATUS" ] || [ "$STATUS" = "null" ] && { echo "배포 상태 추출 실패 — skip"; exit 0; }
