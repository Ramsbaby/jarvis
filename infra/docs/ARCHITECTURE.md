...
**doc-map.json** (`runtime/config/doc-map.json`): 18 patterns mapping code paths → required docs. Shared by PostToolUse debt tracking, Stop enforcement, and `docs-freshness-audit.sh` 의 서술형 드리프트 보고.

`match_glob` 은 이름과 달리 glob 이 아니라 **부분문자열**이다 — `post-tool-docdebt.sh` 가 `mg in file_path or mg in basename` 으로 판정한다. `lib/` 같은 넓은 조각을 넣으면 모든 편집이 세션 종료를 차단하므로 파일 단위로 좁게 적는다.

**2026-05-01 ~ 08-25 (116일) 무력화 사고**: `doc-map.json` 이 `{"patterns":[]}` (16바이트) 로 비어 있었다. 패턴이 0개면 매칭 루프가 0회 돌고, 세 층이 전부 정상 종료한다 — 부채 0건 → `stop-doc-enforce.sh` 가 `no debts — pass`. **`pass` 는 통과가 아니라 측정을 안 한 것이었다.** 원본 18패턴은 git·백업 어디에도 없어 복구 불가였고, 저장소 실측 기반으로 재작성했다.
재발 방지: `post-tool-docdebt.sh` 가 doc-map 부재·패턴 0개를 `logs/doc-debt.log` 에 남기고 종료한다. 조용한 `exit 0` 을 없앤 것이 핵심이다.
...
**Auto-generated docs excluded**: `SYSTEM-OVERVIEW.md` is regenerated by `gen-system-overview.sh` — not subject to manual debt enforcement.

**2026-08-25 경로 일원화**: 생성기는 `runtime/docs/SYSTEM-OVERVIEW.md` 에 쓰는데 `INDEX.md`·`MAP.md` 는 `infra/docs/SYSTEM-OVERVIEW.md` 를 가리키고 있었다. 126일간 매일 `exit 0` · mtime 갱신을 유지하면서 아무도 읽지 않는 파일을 갱신했고, 소비자가 보는 파일은 4월 21일에 멈춰 `${JARVIS_OWNER_NAME:-Your Name}` 이 치환되지 않은 채 굳어 있었다. 정본을 `runtime/docs/`(로컬 전용, `/runtime/*` 는 gitignore) 로 확정하고 공개 저장소의 낡은 사본은 제거, 문서 링크는 로컬 경로 안내로 정정했다.

**exit code 는 성공 판정이 아니다**: 위 두 사고 모두 `exit 0` 이었다. `instrument-liveness-audit.mjs` 가 `instruments.json` 의 `artifact` 항목에 대해 (1) mtime 침묵과 (2) **휘발성 줄을 걷어낸 내용 해시**를 함께 본다. mtime 은 파일을 다시 쓰기만 해도 오르므로, 내용이 `max_unchanged_days` 를 넘겨 그대로면 `🧊 frozen` 으로 승격하고 exit 1 을 반환한다.
