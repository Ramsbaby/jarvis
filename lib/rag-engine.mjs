/**
 * RAG Engine - LanceDB BM25 + Local Embeddings (@xenova/transformers)
 *
 * Hybrid search: BM25 full-text (primary, free, local) + vector similarity (optional enrichment).
 * Local embeddings via Xenova/all-MiniLM-L6-v2 (384-dim, ~90MB, MIT).
...
async function _getEmbedPipeline() {
  if (!_pipelinePromise) {
...
export class RAGEngine {
  constructor(dbPath) {
    ...
    this.db = null;
    this.table = null;
    // openai is kept as null — enrichDocument still uses it if ENABLE_RAG_ENRICHMENT=1
    // Embeddings are now handled locally via @xenova/transformers
    this.openai = null;
  ...
  async refreshTable() {
    try {
      ...
      // table이 없으면 (극히 드문 경우) init() 호출 필요
    }
  }
  ...
  async enrichDocument(text) {
    ...
    // Lazily initialize OpenAI client for enrichment only (not used for embeddings)
    if (!this._openaiEnrich) {
      try {
        const { default: OpenAI } = await import('openai');
        this._openaiEnrich = new OpenAI();
      } catch {
        return DEFAULT;
      }
    }
    ...
    try {
      const response = await this._openaiEnrich.chat.completions.create(
        {
          model: 'gpt-4o-mini',
          messages: [
            {
  ...
  async embed(texts) {
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        const extractor = await _getEmbedPipeline();
        const results = [];
  ...
  async _alertEmbeddingFailure(status, message) {
    ...
    const alertText = `🚨 RAG Embedding 장애 (로컬 모델)\n오류: ${message.slice(0, 200)}\n@xenova/transformers 모델 로드 실패 확인 필요`;
  ...
  async indexFile(filePath) {
    ...
    try {
      ...
      try {
        ...
        const retryQueuePath = join(
          process.env.BOT_HOME || join(homedir(), '.jarvis'),
          'state', 'embedding-retry-queue.jsonl'
        );
        try {
          appendFileSync(
            retryQueuePath,
  ...
  async deleteBySource(source) {
    try {
      ...
      // LanceDB 버그: 빈 테이블이나 매칭 row가 없어도 delete()는 _deletions 파일을 생성하려 하며,
      // 파일 생성 실패 시 이후 query()가 모두 실패함. 먼저 존재 여부 확인 후 skip.
      const existing = await this.table.query()
        .where(`source = '${safeSource}'`).limit(1).toArray().catch(() => []);
      if (existing.length === 0) return;
      await this.table.delete(`source = '${safeSource}'`);
    } catch {
      // Table might be empty or delete failed — skip silently
    }
  }
