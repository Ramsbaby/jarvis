...
export function ensureCronTask(id, meta = {}) {
  ...
  if (!row) {
    ...
    const metaJson = JSON.stringify({ name: meta.name ?? id, source: 'bot-cron', ...meta });
    db.prepare(
      'INSERT INTO tasks (id, status, priority, retries, depends, meta, updated_at) VALUES (?,?,?,?,?,?,?)'
    ).run(id, 'queued', meta.priority ?? 0, 0, '[]', metaJson, now);
    db.prepare(
      'INSERT INTO task_transitions (task_id, from_status, to_status, triggered_by, created_at) VALUES (?,?,?,?,?)'
...
if (process.argv[1]?.endsWith('task-store.mjs')) {
  ...
  try {
    switch (cmd) {
      ...
      // Usage: node task-store.mjs enqueue --id <id> --title <title> --prompt <prompt> [--priority high|medium|low] [--source <src>] [--type <type>]
      ...
      case 'enqueue': {
        ...
        const { id: eId, title, prompt: ePrompt, priority: ePrio = 'medium', source: eSrc = 'agent', type: eType = 'improvement' } = flagMap;
        ...
        const meta = JSON.stringify({ name: title, prompt: ePrompt ?? title, source: eSrc, type: eType, enqueuedAt: now });
        db.prepare('INSERT OR REPLACE INTO tasks (id, status, priority, retries, depends, meta, updated_at) VALUES (?,?,?,?,?,?,?)')
          .run(eId, 'queued', ePrio, 0, '[]', meta, now);
      ...
      case 'ensure': {
        const [id, name, source] = args;
        const result = ensureCronTask(id, { name: name ?? id, source: source ?? 'bot-cron' });
