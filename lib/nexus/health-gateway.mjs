...
" 2>/dev/null || echo "(health.json 없음)"`,
    `echo "=== 네트워크/API ==="`,
    `code=$(curl -s --max-time 3 -o /dev/null -w "%{http_code}" https://api.anthropic.com 2>/dev/null); if [ -z "$code" ] || [ "$code" = "000" ]; then echo "Anthropic API: ❌ Unreachable"; elif [ "$code" = "429" ]; then echo "Anthropic API: ⚠️ Rate Limited (HTTP 429)"; elif [ "$code" -ge 200 ] && [ "$code" -le 299 ]; then echo "Anthropic API: ✅ OK (HTTP $code)"; elif [ "$code" -ge 400 ] && [ "$code" -le 499 ]; then echo "Anthropic API: ⚠️ Client Error (HTTP $code)"; elif [ "$code" -ge 500 ] && [ "$code" -le 599 ]; then echo "Anthropic API: ❌ Server Error (HTTP $code)"; else echo "Anthropic API: ❌ Unreachable"; fi`,
    `echo "=== 스토리지 ==="`,
