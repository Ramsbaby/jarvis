...
" 2>/dev/null || echo "(health.json 없음)"`,
    ...
    if (name === 'rag_search') {
      ...
      if (!query || !query.trim()) {
        return { content: [{ type: 'text', text: 'query가 비어있습니다.' }], isError: true };
      }
      try {
        ...
        if (results.length === 0) {
          return { content: [{ type: 'text', text: `"${query}" 관련 기억 없음.` }] };
        }
        ...
        return { content: [{ type: 'text', text: `검색: "${query}" → ${results.length}개\n\n${formatted}` }] };
      } catch (err) {
        return { content: [{ type: 'text', text: `RAG 검색 오류: ${err.message}` }], isError: true };
      }
    }
