...
check_cron_errors() {
  ...
  if [[ -n "$cutoff" ]]; then
    err_count=$(grep -E 'FAILED|ERROR|CRITICAL' "$BOT_HOME/logs/cron.log" 2>/dev/null \
      | awk -v c="[$cutoff" '$0 >= c' | wc -l | tr -d ' ' || echo "0")
  else
    err_count=$(grep -cE 'FAILED|ERROR|CRITICAL' "$BOT_HOME/logs/cron.log" 2>/dev/null | tr -d ' ' || echo "0")
  fi
...
# ── 결과 포맷팅 ───────────────────────────────────────────────────────────────
if [[ "$wf" -eq 0 ]]; then
  MSG="✅ Jarvis 시스템 정상 — $(date '+%m-%d %H:%M')  |  OK ${ok}개"
  log "$MSG"
  if [[ -f "$ROUTE" ]]; then
    "$ROUTE" discord system-doctor "$MSG" jarvis-system 2>/dev/null || true
  fi
  exit 0
fi
...
if [[ -f "$ROUTE" ]]; then
  "$ROUTE" discord system-doctor "$REPORT" jarvis-system 2>/dev/null || true
fi
